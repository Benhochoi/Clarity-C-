...
// Dieu kien toan so, bat dau bang '0'
...
// them ham nay vao trong class StatsService (sau cac ham khac)

// chuyen chuoi ngay dd/mm/yyyy thanh so nguyen YYYYMMDD de so sanh
static int chuyenNgayThanhSo(const string& ngay) {
    ...
    // chuyen thanh YYYYMMDD
    return stoi(yyyy) * 10000 + stoi(mm) * 100 + stoi(dd);
}

// bao cao doanh thu theo khoang thoi gian tu ngay a den ngay b
static void doanhthutheokhoangtg(const vector<HoaDon>& ds) {
    string ngaybatdau, ngayketthuc;
    
    cout << "BAO CAO DOANH THU THEO KHOANG THOI GIAN";
    ...
    // chuyen doi ngay thanh so de so sanh
    int sobatdau = chuyenNgayThanhSo(ngaybatdau);
    int soketthuc = chuyenNgayThanhSo(ngayketthuc);
    ...
    // map luu doanh thu theo tung ngay trong khoang
    map<string, double> doanhthutheo_ngay;
    double tongdoanhthu = 0;
    int sohoadon = 0;
    
    // duyet danh sach hoa don
    for (const HoaDon& hd : ds) {
        int songayhd = chuyenNgayThanhSo(hd.getNgayLap());
        
        // kiem tra xem hoa don co nam trong khoang thoi gian khong
        if (songayhd >= sobatdau && songayhd <= soketthuc) {
            doanhthutheo_ngay[hd.getNgayLap()] += hd.getThanhToan();
            tongdoanhthu += hd.getThanhToan();
            sohoadon++;
        }
    }
    ...
    cout << " Tu ngay " << ngaybatdau << " den " << ngayketthuc << "\n";
    cout << left << setw(15) << "Ngay" 
         << right << setw(20) << "Doanh thu (VND)" 
         << setw(15) << "So hoa don" << '\n';
    cout << string(50, '-') << '\n';
    
    map<string, int> sohdtheo_ngay;
    for (const HoaDon& hd : ds) {
        int songayhd = chuyenNgayThanhSo(hd.getNgayLap());
        if (songayhd >= sobatdau && songayhd <= soketthuc) {
            sohdtheo_ngay[hd.getNgayLap()]++;
        }
    }
    
    for (map<string, double>::const_iterator it = doanhthutheo_ngay.begin(); 
         it != doanhthutheo_ngay.end(); ++it) {
        cout << left << setw(15) << it->first
             << right << setw(20) << fixed << setprecision(0) << it->second
             << setw(15) << sohdtheo_ngay[it->first] << '\n';
    }
    
    cout << string(50, '-') << '\n';
    cout << left << setw(15) << "TONG CONG:"
         << right << setw(20) << fixed << setprecision(0) << tongdoanhthu
         << setw(15) << sohoadon << '\n';
    
    // tinh doanh thu trung binh
    double trungbinh = tongdoanhthu / sohoadon;
    cout << "Doanh thu trung binh: " << fixed << setprecision(0) 
         << trungbinh << " VND/hoa don";
    cout << "============================================================\n";
}
};
                 ...
                 const vector<KhachHang>& dsKH)
{
    ...
    do{
        cout << "\n========== THONG KE & BAO CAO ==========\n"
			 ...
			 << "6. Bao cao doanh thu theo khoang thoi gian"            
             << "0. Quay lai\n"
             << "========================================\n"
             << "Chon: " << flush;
        ...
        switch(c){
            ...
            case 6:{
        		StatsService::doanhthutheokhoangtg(dsHD);
        		}
