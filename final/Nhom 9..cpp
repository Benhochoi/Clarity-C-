...
static void DoanhThuTheoKhoang(const vector<HoaDon>& ds) {
    cin.ignore();
    ...
    cout << "\n========== HOA DON TRONG KHOANG " <<ngayBatDau 
         << " -> " << ngayKetThuc << " ==========\n";
    cout << left << setw(12) << "Ma HD"
         << setw(15) << "Ngay lap"
         << setw(15) << "Ma KH"
         << right << setw(20) << "Thanh toan (VND)" << '\n';
    cout << string(62, '-') << '\n';
    
    for (const HoaDon& hd : ds) {
        const string& ngayLap = hd.getNgayLap();
        
        if (ngayLap >= ngayBatDau && ngayLap <= ngayKetThuc) {
            cout << left << setw(12) << hd.getMaHD()
                 << setw(15) << ngayLap
                 << setw(15) << hd.getMaKH()
                 << right << setw(20) << fixed << setprecision(0) 
                 << hd.getThanhToan() << '\n';
    ...
    cout << string(62, '=') << '\n';
    cout << "Tong so hoa don: " << soHoaDon << '\n';
    cout << "Tong doanh thu: " << fixed << setprecision(0) 
         << tongDoanhThu << " VND\n";
    
    if (soHoaDon > 0) {
        cout << "Doanh thu trung binh: " 
             << tongDoanhThu / soHoaDon << " VND/hoa don\n";
    }
}
                 ...
                 const vector<KhachHang>& dsKH)
{
    ...
    do{
     ...
     << "4. Top N thuoc ban chay\n"              
     << "5. Thong ke khach hang\n"               
     << "6. Thuoc sap het hang\n"                
     << "0. Quay lai\n"
     << "========================================\n"
     << "Chon: " << flush;        
...
switch(c){
    ...
    case 2: 
        ...
        break;
    case 3:
        StatsService::DoanhThuTheoKhoang(dsHD);
        break;
    case 4: {  
        int n; 
        cout << "Nhap N: " << flush; 
    ...
    case 5:  // Ð?I T? case 4 -> case 5
        StatsService::ThongKeKhachHang(dsKH); 
        break;
    case 6: {  // Ð?I T? case 5 -> case 6
        int nguong;
        cout << "Nhap nguong toi thieu (mac dinh 10): ";
        cin >> nguong;
        if (nguong < 0) nguong = 10;
